...
export function DeleteProjectButton({ action }: DeleteProjectButtonProps) {
  return (
    <ConfirmProjectActionButton
      ...
      className="inline-flex h-9 w-9 items-center justify-center rounded-xl border border-[var(--line)] bg-[var(--soft)] text-[var(--ink)] transition duration-200 hover:-translate-y-0.5 hover:border-[var(--brand)] hover:text-[var(--brand)] disabled:opacity-60"
    />
  );
}
